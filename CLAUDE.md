...
**Vision board to come back to:** `docs/vision-board.md` (pictures in `docs/mockups/vision/`). Home card work in progress lives in `mock-posh.html` (built by scratch scripts; not committed). Product facts are in `PRODUCT.md`.

**Latest (branch `v5`, the new Home):** Home is one card plus today's list. The card front: date, a big number (done of total), three small LED lights (Body green, Mind violet, To-do amber) that brighten as things get done and glow when an area is complete, and one kind sentence. Tapping a light filters the list to that area; tapping elsewhere turns the card. The back: this week in lights (three strips of seven LEDs), days fully lit, and the blob saying one line. When everything is done the card "switches on" (glow, "all done"). Trackers (body logs, limits, Body measures like water and sleep) are not counted and not listed; they live in the "Log or add" sheet behind the centre plus. Split bottom menu: Patterns, Home, plus, Day, Settings (user finds it a bit cheap: redesign later). One fasting row under the card opens in place into an arc with a live timer and a sun or moon; End fast asks save or discard. Patterns calendar is frosted glass month tiles. A soft top light follows the time of day. Date arrows move one day. Code: one plain block in index.html starting at `var DOW_LONG` (card: hmHeroHtml, qcBar, qcSay, qcBackWeek, qcTurn; Log or add: qcLogHtml; fasting row and arc: qfStrip, fastHomeCard, qfEndSheet; Patterns glass calendar: qcGlassCal); hmBody draws the card, list, fasting row and blob; isTracker(i) sits next to isLimit and dailyRoutine skips trackers, so the score, the card back and Patterns agree. The CSS is still layered (later v5 rules refine earlier ones, some !important); dead Home code (hmCardsHtml, hmArc, hmSpark, hmNextHtml, progressCard) can go later. Fasting page redesigned (viewFasting and the fp* helpers): live arc or a start panel, stage bar, range tabs 7d 30d 90d 1y All driving the stats, a "When you fast" weekday by hour heat grid, stages reached, records, recent fasts. No weight or fat estimates on purpose. Next agreed: tidy the merged mock code and fix dayScore, then the bottom navigation. sw.js cache `ritsu-v24`.

**Latest (built, committed locally until the user says push):** "things that come back" and the one minute setup. Engine is one block in index.html starting `CB_LIB` (search "things that come back"): each thing is ONE live one-off To-do task carrying a `cb` object (`kind` visit or simple, `every` months, `rule` since or fixed, `stage` quiet or booked, `next`, `booked`, `at`, `snooze`, `last`); its own `dueDate` is the day it shows up, so every screen already handles it. Visit shape = Book it (2 weeks before due), Booked (typed like "friday 10am" through `cbBookSheet`), Did you go? (after the time, `cbMode` = went), Quiet again. Simple shape = Done, Quiet again. The companion asks one question at a time on Home (`cbAskHtml`, card under Up next) and rows on the Day list carry the same buttons (`cbRowExtra`); Later snoozes 3 days, never red. Welcome now says Set me up (`setupS`, `setupHtml`, `setupFinish`): step 2 habit tiles, step 3 six switches with one question each; Start empty and Restore stay as small links. Typing "renew passport every 2 years" makes one that comes back. Settings has a Things that come back panel (`cbPanel`). Not built: Full day pop-up, clash warning, the library word-to-tag suggestions, Default list Personal + starter tags, Get the results follow-up. sw.js cache `ritsu-v21`. **Before that:** Branch `v4` on GitHub, last commit `eceb171` (batch 1 wrong numbers and polish, batch 2 slimmer Home). Since `b80b0c9`: Home shows only today (cards say "N to go", tap turns over, tap again turns back, hold opens the list, "Open list" strip on the back); the calendar, insights and fasting card moved to Patterns (was Reports); areas, People and Fasting links moved to Settings; four-button menu with the plus beside it; Welcome has one primary choice. Design audit results and pictures: `docs/mockups/mock-home2.png`, `mock-setup.png`, `mock-comesback.png`, `mock-light-laptop.png`. NOT built yet from the plan: the 1-minute setup, tasks that come back (dentist), full-day pop-up, clash warning, Year view whole-year score, a working state on the Welcome buttons, vague Back/Save/Done labels and heading order. sw.js cache is `ritsu-v19`. Old line follows: Branch `v4` on GitHub, last commit `b80b0c9`. The live site still serves `v2`; `main`, `v2`, `v3` are untouched. v4 has: compact cards, pill nav, the companion blob, the pull cord, undoable deletes, the add sheet (Date, Time wheel, Priority, Repeat, Tags, Note, Length), and typing a sentence ("call alex tom at 5 in the evening"). This file and `docs/` exist only on the user's computer, not on GitHub.
...
- **Fonts:** `Space Grotesk` everywhere (text, headings and numbers; `--ui` and `--serif` both point to it), chosen by the user from the Intermittent fasting app (intermittentfast.ing). Big thin numbers use its lightest weight, 300. Loaded non-blocking from Google Fonts. Bricolage Grotesque and Instrument Sans were replaced on 2026-09-26.
